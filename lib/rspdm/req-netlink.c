...
int spdm_netlink_sig_event(struct device *dev,
			   struct spdm_artifacts atrifacts,
			   int rsp_code, u8 slot,
			   size_t req_nonce_off, size_t rsp_nonce_off,
			   const char *spdm_context)
{
	const void *transcript = atrifacts.transcript;
	size_t transcript_len = atrifacts.transcript_len;
	const void *cert_chain = atrifacts.cert_chain[slot];
	size_t cert_chain_len = atrifacts.cert_chain_len[slot];
	enum hash_algo base_hash_alg = atrifacts.base_hash_alg;
	size_t sig_len = atrifacts.sig_len;
	u8 version = atrifacts.version;
